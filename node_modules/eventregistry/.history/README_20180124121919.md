...
er.getConceptUri("George Clooney").then((conceptUri) => {
    const q = new erBase.QueryArticlesIter(er, {conceptUri: conceptUri, sortBy: "date"});
    q.execQuery((items) => {
...
const er = new erBase.EventRegistry({apiKey: "YOUR_API_KEY"});

er.getConceptUri("Star Wars").then((conceptUri) => {
    const q = new erBase.QueryEvents({conceptUri: conceptUri});
    const requestEventsInfo = new erBase.RequestEventsInfo({sortBy: "date", count: 10});
